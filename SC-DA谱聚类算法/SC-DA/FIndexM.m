%评价指标F指标
function [Findex]=FIndexM(A,IDX,patternNum,k)
...
%计算不同类之间的点对总数all_diff，以及同一类之间的点对数
all_diff=0;
all_same=0;
for i=1:k_new
    all_diff=all_diff+number_new(i)*(patternNum-number_new(i))/2;
    all_same=all_same+number_new(i)*(number_new(i)-1)/2;
end
%计算原始不同类的对象被划分到不同类的点对的数目
TN=all_diff-FN;
FP=all_same-TP;
%计算准确率
precision=TP/all_same;
%计算召回率
recall=TP/(TP+FN);
%计算FIndex评价指标值
Findex=2*precision*recall/(precision+recall);
