...
clear all
clc
number_loop=20;%循环的次数
[X,number_sample,k,class_original]=InputData('D:\datasetNS\dataset1.txt');%输入数据，得到样本个数、类别数
Standard_Deviation=0.001;%人工赋值标准差
...
for i=1:number_loop  
    t1=clock;
    [rad_density]=GetRad(X,number_sample);%计算得到所需要的参数，密度半径
    [CNN]=CommonNearNeighbor(X,number_sample,rad_density);%计算样本和其近邻的交集中的样本个数
    [A]=AffinityMatrix(X,number_sample,k,Standard_Deviation,CNN);%step1：根据数据集X，得到相似矩阵A
...
mean_rate_correct=sum(rate_correct)/number_loop;%计算平均错误率 
